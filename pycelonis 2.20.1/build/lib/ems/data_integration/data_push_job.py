...
from pycelonis.service.integration.service import DataPushChunk
from pycelonis.service.integration.service import DataPushJob as DataPushJobBase
from pycelonis.service.integration.service import IntegrationService, JobStatus
...
class DataPushJob(DataPushJobBase):
    ...
    def sync(self) -> None:
        ...
        synced_job = IntegrationService.get_api_v1_data_push_pool_id_jobs_id(self.client, self.data_pool_id, self.id)
        self._update(synced_job)

    def delete(self) -> None:
        """Deletes data push job."""
        IntegrationService.delete_api_v1_data_push_pool_id_jobs_id(self.client, self.data_pool_id, self.id)
    ...
    def add_file_chunk(self, file: typing.Union[io.BytesIO, io.BufferedReader]) -> None:
        ...
        IntegrationService.post_api_v1_data_push_pool_id_jobs_id_chunks_upserted(
    ...
    def add_data_frame(self, df: pd.DataFrame, chunk_size: int = 100_000, index: typing.Optional[bool] = False) -> None:
        ...
        for i in tqdm(range(0, df.shape[0], chunk_size), disable=Config.DISABLE_TQDM):
            with io.BytesIO() as f:
                try:
                    write_parquet(df.iloc[i : i + chunk_size], f, index=index)
                    f.seek(0)  # Reset position
                    self.add_file_chunk(f)
    ...
    def get_chunks(self) -> CelonisCollection[typing.Optional[DataPushChunk]]:
        ...
        return CelonisCollection(
            IntegrationService.get_api_v1_data_push_pool_id_jobs_id_chunks(self.client, self.data_pool_id, self.id)
        )
    ...
    def execute(self, wait: bool = True) -> None:
        ...
        IntegrationService.post_api_v1_data_push_pool_id_jobs_id(self.client, self.data_pool_id, self.id)
    ...
    def _wait_for_execution(self) -> None:
        ...
        def format_status(data_push_job_transport: DataPushJobBase) -> str:
            if not data_push_job_transport.status:
                return ""
            return f"Status: {data_push_job_transport.status.value}"
    ...
    def _get_status(self) -> DataPushJobBase:
        return IntegrationService.get_api_v1_data_push_pool_id_jobs_id(self.client, self.data_pool_id, self.id)
