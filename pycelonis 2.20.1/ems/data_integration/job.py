...
class Job(JobTransport):
    ...
    def create_transformation(
        ...
        Examples:
            ...
            ```
        """
        return self.create_task(name, TaskType.TRANSFORMATION, description=description, **kwargs)
    ...
    def create_extraction(self, name: str, **kwargs: typing.Any) -> "Task":
        ...
        """
        return self.create_task(name, TaskType.EXTRACTION, **kwargs)
    ...
    def execute(
        ...
        mode: ExtractionMode = ExtractionMode.DELTA,
        wait: bool = True,
        **kwargs: typing.Any,
    ) -> None:
    ...
    def _verify_execution_possible(self) -> None:
        if self._is_execution_in_progress(self._get_current_execution_status()):
    ...
    def _wait_for_execution(self) -> None:
        ...
        def format_current_execution_status(
            ...
            if current_execution_status.status:
                formatted_execution_status += f" {current_execution_status.status.value}"
            return formatted_execution_status

        logger.info("Wait for execution of job with id '%s'", self.id)
        poll(
            target=self._get_current_execution_status,
    ...
    def _verify_execution_successful(self) -> None:
        current_execution_status = self._get_current_execution_status()
        if current_execution_status.status != ExecutionStatus.SUCCESS:
            ...
            if len(str_detailed_error_log) == 0:
                error_message = current_execution_status.status.value if current_execution_status.status else ""
            else:
                error_message = str_detailed_error_log
            raise PyCelonisExecutionFailedError(error_message)

    def _get_current_execution_status(self) -> EntityStatus:
