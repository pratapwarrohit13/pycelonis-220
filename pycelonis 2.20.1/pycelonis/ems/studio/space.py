...
class Space(SpaceTransport):
    ...
    def update(self) -> None:
        ...
        updated_space = PackageManagerService.put_api_spaces_id(
            self.client, self.id, SpaceDetailsTransport(**self.json_dict())
        )
    ...
    def delete(self, soft_delete_packages: typing.Optional[bool] = False) -> None:
        """Deletes space.

        Args:
            soft_delete_packages: False by default, set to True for soft-deletion for the packages under the space
        """
        PackageManagerService.post_api_spaces_delete_id(
            self.client, self.id, SpaceDeleteTransport(**self.json_dict()), soft_delete_packages=soft_delete_packages
        )
