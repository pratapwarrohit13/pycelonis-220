...
```python
data_model = data_pool.get_data_model(
    data_model_id
)
data_model.name = (
    "NEW_NAME"
...
from pycelonis.ems.data_integration.data_export import DataExport, DataExportV2
...
from pycelonis.service.integration.service import (
    ...
    ProxyDataExportRequestV2,
    ProxyQueryEnvironmentV2,
    QueryEnvironment,
)
...
class DataModel(
    ...
    def _wait_for_reload(self) -> None:
        ...
        def format_current_compute_load(
            ...
            if current_compute_load.load_status:
                formatted_compute_load_status += f" {current_compute_load.load_status}"
    ...
    def _verify_load_successful(self) -> None:
        ...
        if current_compute_load and current_compute_load.load_status == DataModelLoadStatus.WARNING:
            logger.warning("%s: %s", current_compute_load.load_status, current_compute_load.message)
    ...
    def export_data_frame_from(
        ...
        query_environment: typing.Optional[typing.Union[QueryEnvironment, ProxyQueryEnvironmentV2]] = None,
        use_api_v2: bool = False,
    ) -> pd.DataFrame:
        """Creates new data export and downloads exported data as data frame for given data model.

        !!! warning
            The method `DataModel.export_data_frame_from` has been deprecated and will be removed in
            future
        ...
        Args:
            ...
            use_api_v2: Use new version of data export api
        ...
        return data_model.export_data_frame(query, query_environment=query_environment, use_api_v2=use_api_v2)
    ...
    def create_data_export(
        ...
        query_environment: typing.Optional[typing.Union[QueryEnvironment, ProxyQueryEnvironmentV2]] = None,
        use_api_v2: bool = False,
    ) -> typing.Union["DataExport", "DataExportV2"]:
        """Creates new data export in given data model.

        Args:
            query: PQL query to export.
            export_type: Export type.
            query_environment: Query environment KPIs, mandatory for V2.
            use_api_v2: Use new version of data export api
        ...
        try:
            if use_api_v2:
                return self._export_data_v2(query, export_type, query_environment)  # type: ignore

            return self._export_data_v1(query, export_type, query_environment)  # type: ignore
        except PyCelonisPermissionError as e:
    ...
    def export_data_frame(
        ...
        !!! warning
            The method `data_model.export_data_frame` has been deprecated and will be removed in future
            versions. Please use SaolaPy from now on to export PQL queries:
                ```python
                import pycelonis.pql as pql
        ...
        Examples:
            ...
            query += PQLColumn(
                name="_CASE_KEY",
                query='"ACTIVITIES"."_CASE_KEY"',
            )
            query += PQLColumn(
                name="ACTIVITY_EN",
                query='"ACTIVITIES"."ACTIVITY_EN"',
            )
        ...
        df = self._export_data_frame(query, query_environment, use_api_v2=use_api_v2)
        internal_tracking_logger.debug(
            "Exported DataFrame.",
            extra={"nrows": df.shape[0], "ncolumns": df.shape[1], "tracking_type": "DATA_MODEL_EXPORT"},
        )
        return df

    def _export_data_frame(
        self,
        query: typing.Union[DataQuery, PQL],
        query_environment: typing.Optional[typing.Union[QueryEnvironment, ProxyQueryEnvironmentV2]] = None,
        use_api_v2: bool = False,
        ...
        data_export = self.create_data_export(
            query,
            ExportType.PARQUET,  # type: ignore
    ...
    def _export_data_v1(
        ...
        logger.info("Successfully created data export using api v1 with id '%s'", data_export_transport.id)
    ...
    def _export_data_v2(
        ...
        query: typing.Union[DataQuery, PQL],
        export_type: ExportType,
        query_environment: typing.Optional[ProxyQueryEnvironmentV2] = None,
    ) -> DataExportV2:
        if query_environment is None:
            query_environment = ProxyQueryEnvironmentV2(saved_formulas=[])
        if not isinstance(query, (DataQuery, PQL)):
            raise TypeError(
                f"Parameter `query` is of wrong type for data export using api v2: "
                f"{type(query)}, needs to be `DataQuery` or, `PQL`."
            )
        if not isinstance(query_environment, ProxyQueryEnvironmentV2):
            raise TypeError(
                f"Parameter `query_environment` is of wrong type for data export using api v2: "
                f"{type(query_environment)}, needs to be `ProxyQueryEnvironmentV2`."
            )

        queries = query.queries  # type: ignore
        joined_query = " ;".join(queries)  # type: ignore
        proxy_data_export_request = ProxyDataExportRequestV2(
            export_type=export_type, queries=joined_query, query_environment=query_environment
        )
        data_export_transport = IntegrationService.post_api_external_compute_data_models_data_model_id_query_exports(
            self.client, self.id, proxy_data_export_request
        )
        logger.info("Successfully created data export using api v2 with id '%s'", data_export_transport.id)
        return DataExportV2.from_transport(self.client, self.id, data_export_transport)
