...
class AnalysisComponent(PyCelonisBaseModel, extra=Extra.allow):
...
class DataComponent(ABC, AnalysisComponent):
    ...
    def get_query(self, use_translated_names: bool = True) -> PQL:
        ...
        """
        query = PQL()
        query += self.get_columns(use_translated_names)

        if component_filter := self.get_filter():
            query += component_filter
        return query

    def get_columns(self, use_translated_names: bool = True) -> typing.List[PQLColumn]:
        ...
        Examples:
            ...
            ```
        """
        return [self._build_query_column(raw_column, use_translated_names) for raw_column in self._raw_columns]
...
class PQLTable(DataComponent):
    ...
    def get_query(self, use_translated_names: bool = True) -> PQL:
        ...
        Examples:
            ...
            ```
        """
        query = super().get_query(use_translated_names)
